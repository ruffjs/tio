<template>
  <div class="subscriptions-form" :class="{ visible }">
    <el-card shadow="never" class="subscriptions-form-card">
      <template #header>
        <div class="subscriptions-form-header">
          <el-button icon="ArrowLeftBold" text @click="emit('cancel')">Cancel</el-button>
          <span>{{ isCreate ? "Add Subcription" : "Edit Subcription" }}</span>
          <el-button type="primary" text @click="submitForm">Subscribe</el-button>
        </div>
      </template>
      <el-row :gutter="20" class="subscriptions-form-main">
...
const visible = computed(() => isEdit.value || isCreate.value);
const isThing = computed(() => props.connConfig?.userrole === "thing");
const topics = computed(() => {
  if (isThing.value) {
    return thingSubTopics.map(({ name, topicResolver }) => ({
      name,
      topic: topicResolver({ thingId: props.connConfig?.username }),
    }));
  }
...
watch(
  ...
  () => {
    ...
    } else if (isEdit.value && props.data) {
      const { id, keep, topic, opts, name } = props.data;
      Object.assign(form, {
        id,
        topic,
        qos: opts.qos,
